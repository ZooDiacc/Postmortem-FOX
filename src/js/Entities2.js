...
Player = function() {
  ...
  let obstacles = [{ xStart: 305, yStart: 285, xEnd: 525, yEnd: 425 }];
  ...
  self.updatePosition = function() {
    ...
    if (self.pressingLeft && (self.x > 105 || (self.y > 395 && self.y < 465)))
      newX -= 10;
    if (self.pressingDown && self.y < 685) newY += 10;
...
Enemy.randomlyGenerate = function() {
  ...
  var width = 64;
  var id = Math.random();
  if (Math.random() < 0.5) Enemy(id, x, y, 100, 100, Img.bat, 5, 2);
  else Enemy(id, 200, 300, width, height, Img.DemonSprite, 1, 3);
};
